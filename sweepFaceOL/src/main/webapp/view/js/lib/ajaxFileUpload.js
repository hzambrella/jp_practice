...
$.extend({
    ...
    createUploadForm: function (id, fileElementId) {
        ...
        // old string param for single file
        if (typeof (fileElementId) == 'string') {
            fileElementId = [fileElementId];
        }
        var $form=$('<form>').attr({id:formId,name:formId}).css({position:'absolute',top:'-1200px',left:'-1200px'})
        for (var i in fileElementId) {
            var oldElement = $('#' + fileElementId[i]),newElement = $(oldElement).clone();
    ...
    addOtherRequestsToForm: function (form, data) {
        ...
        for (var key in data) originalElement.clone().attr({
            'name': key,
            'value': data[key]
        }).appendTo(form);
        return form;
    ...
    ajaxFileUpload: function (s) {
        ...
        var id = new Date().getTime(),
            io = $.createUploadIframe(id),
            form = $.createUploadForm(id, s.fileElementId),
