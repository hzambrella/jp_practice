	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			...
			if (formItems != null && formItems.size() > 0) {
				...
				File imgFile = new File(request.getServletContext()
						.getRealPath("")
						+ File.separator
				...
				// 调用face++接口
				try {
					DetectRespSucc re = KuangshiOL
							.kuangshiFaceDetectPostByHttpClient(imgFile);

					if (null == re) {
						result.setCode(500);
						result.setMessage("服务接口异常");
						response.getWriter().println(result.toJSON());
						System.out.println("log[warn]face++:服务接口异常 re is null");
						return;
					}
					...
					if (null != re.getErrorMessage()) {
						...
						response.getWriter().println(result.toJSON());
						System.out
								.println("log[warn]face++:服务接口异常 have error message");
						return;
					}
